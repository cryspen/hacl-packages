...
#[test]
fn self_test() {
    let pk = PublicKey::new(KeySize::N2048, &MODULUS).unwrap();
    let sk = PrivateKey::new(&pk, &PRIVATE_EXPONENT).expect("Error building private key");
    let salt = [1, 2, 3, 4, 5];
    let msg = [7, 8, 9, 10];
    let signature = sign(Sha256, &sk, &salt, &msg).unwrap();
    println!("signature: {:x?}", signature);
    verify(Sha256, &pk, &signature, &msg, salt.len()).expect("Error verifying signature");
}
...
fn wycheproof_single_test() {
    let pk = PublicKey::new(KeySize::N2048, &N).unwrap();
    let msg: [u8; 0] = [];
    ...
    verify(Sha256, &pk, &signature, &msg, 0).expect("Error verifying signature");
    ...
    verify(Sha256, &pk, &signature, &msg, 32).expect("Error verifying signature");
}
...
fn test_wycheproof() {
    ...
    fn run(tests: RsaPssTestVector) {
        ...
        for testGroup in tests.testGroups.iter() {
            ...
            let public_key = PublicKey::new(key_size, &n)
            ...
            for test in testGroup.tests.iter() {
                ...
                let result = verify(hash_algorithm, &public_key, &signature, &msg, salt_len);
                if valid {
                    assert!(result.is_ok());
                } else {
