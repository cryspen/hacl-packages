...
fn main() {
    ...
    let mut cfg = BuildConfig::new("gcc-compatible", "hacl_static", cross);
    ...
    // println!("hacl lib dir: {:?}", hacl_lib_path);
    let hacl_include_path = hacl_path.join("include").join("hacl");
    // println!("hacl include dir: {:?}", hacl_include_path);
    ...
    println!(
        ...
        hacl_lib_path.join(cfg.lib_name).display()
    );
}
