...
class Config:
    ...
    def __init__(
        ...
        self.libcrux_benchmarks = self.config["libcrux_benchmarks"]
        ...
        # Flatten benchmark sources
        self.libcrux_benchmark_sources = [
            f for files in [self.libcrux_benchmarks[b] for b in self.libcrux_benchmarks] for f in files
        ]
    ...
    def write_cmake_config(self, cmake_config):
        ...
        with open(cmake_config, "w") as out:
            ...
            out.write(
                "set(LIBCRUX_BENCHMARK_SOURCES\n\t%s\n)\n"
                % (
                    "\n\t".join(
                        join("${PROJECT_SOURCE_DIR}", "benchmarks", f)
                        for f in self.libcrux_benchmark_sources
                    ).replace("\\", "/")
                )
            )
