...
#define HACL_SIGNATURE_ECDSA_P384_SECRETKEY_LEN 48

#define HACL_SIGNATURE_ECDSA_P384_PUBLICKEY_LEN 96
#define HACL_SIGNATURE_ECDSA_P384_PUBLICKEY_COMPRESSED_LEN 49
#define HACL_SIGNATURE_ECDSA_P384_PUBLICKEY_UNCOMPRESSED_LEN 97

#define HACL_SIGNATURE_ECDSA_P384_NONCE_LEN 48

#define HACL_SIGNATURE_ECDSA_P384_SIGNATURE_LEN 96
...
TEST(ApiSuite, ApiTest)
{
  ...
  // Keys
  uint8_t sk[HACL_SIGNATURE_ECDSA_P384_SECRETKEY_LEN];
  uint8_t pk[HACL_SIGNATURE_ECDSA_P384_PUBLICKEY_LEN];

  // Note: This function is not in HACL*.
  //       You need to bring your own keys.
  generate_p384_keypair(sk, pk);

  // Nonce
  uint8_t nonce[HACL_SIGNATURE_ECDSA_P384_NONCE_LEN];

  // Signature
  uint8_t signature[HACL_SIGNATURE_ECDSA_P384_SIGNATURE_LEN];

  // Sign
  bool res_sign = Hacl_P384_ecdsa_sign_p384_sha2(
  ...
  // Verify
  bool res_verify = Hacl_P384_ecdsa_verif_p384_sha2(
    message_size, (uint8_t*)message, pk, signature, signature + 48);
...
class P384EcdsaWycheproof : public ::testing::TestWithParam<TestCase>
{};

TEST_P(P384EcdsaWycheproof, TryWycheproof)
{
  ...
  if (compressed_point) {
    ...
    Hacl_P384_raw_to_compressed(plain_public_key, got_compressed.data());
    ASSERT_EQ(test_case.public_key, got_compressed);
  } else if (uncompressed_point) {
    bytes got_uncompressed(test_case.public_key.size());
    Hacl_P384_raw_to_uncompressed(plain_public_key, got_uncompressed.data());
...
INSTANTIATE_TEST_SUITE_P(
  WycheproofSha256,
  P384EcdsaWycheproof,
  ::testing::ValuesIn(read_json("ecdsa_secp384r1_sha384_test.json")));

INSTANTIATE_TEST_SUITE_P(
  WycheproofSha512,
  P384EcdsaWycheproof,
  ::testing::ValuesIn(read_json("ecdsa_secp384r1_sha512_test.json")));
