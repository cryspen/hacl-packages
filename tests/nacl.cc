...
const size_t NACL_TAG_SIZE = 16;
const size_t NACL_crypto_box_BEFORENMBYTES = 32;
...
TEST_P(NaClBoxSuite, EasyKAT)
{
  ...
  // Encrypt (Alice)
  {
    bytes got_ciphertext(NACL_TAG_SIZE + test.plaintext.size());
...
TEST_P(NaClBoxSuite, EasyDetachedKAT)
{
  ...
  bytes expected_tag =
    bytes(test.ciphertext.begin(), test.ciphertext.begin() + NACL_TAG_SIZE);
  bytes expected_ciphertext =
    bytes(test.ciphertext.begin() + NACL_TAG_SIZE, test.ciphertext.end());

  // Encrypt
  {
    bytes got_tag(NACL_TAG_SIZE);
...
TEST_P(NaClBoxSuite, SplitKAT)
{
  ...
  bytes k(NACL_crypto_box_BEFORENMBYTES);
