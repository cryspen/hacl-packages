...
// ANCHOR(DEFINE)
#define HACL_DH_P521_SECRETKEY_LEN 66
#define HACL_DH_P521_PUBLICKEY_LEN 132
#define HACL_DH_P521_SHARED_LEN 132
// ANCHOR_END(DEFINE)
...
TEST(ApiSuite, ApiTest)
{
  ...
  uint8_t alice_sk[HACL_DH_P521_SECRETKEY_LEN];
  uint8_t alice_pk[HACL_DH_P521_PUBLICKEY_LEN];
  // Note: This function is not in HACL*.
  //       You need to bring your own random.
  generate_p521_keypair(alice_sk, alice_pk);

  // ... and Bob does as well.
  uint8_t bob_sk[HACL_DH_P521_SECRETKEY_LEN];
  uint8_t bob_pk[HACL_DH_P521_PUBLICKEY_LEN];
  // Note: This function is not in HACL*.
  //       You need to bring your own random.
  generate_p521_keypair(bob_sk, bob_pk);

  // Now, Alice and Bob exchange their public keys so that
  // Alice can compute her shared secret as ...
  uint8_t shared_alice[HACL_DH_P521_SHARED_LEN];
  bool res_alice = Hacl_P521_dh_responder(shared_alice, bob_pk, alice_sk);

  // ... and Bob can compute his shared secret as ...
  uint8_t shared_bob[HACL_DH_P521_SHARED_LEN];
  bool res_bob = Hacl_P521_dh_responder(shared_bob, alice_pk, bob_sk);
  ...
  EXPECT_TRUE(memcmp(shared_alice, shared_bob, HACL_DH_P521_SHARED_LEN) == 0);
...
read_json()
{
  ...
  std::string test_dir = "ecdh_secp521r1_ecpoint_test.json";
...
class P521EcdhWycheproof : public ::testing::TestWithParam<TestCase>
{};

TEST_P(P521EcdhWycheproof, TryWycheproof)
{
  ...
  uint8_t plain_private_key[66] = { 0 };
  size_t sk_len = test_case.private_key.size();
  if (sk_len > 66) {
    sk_len = 66;
  }
  for (size_t i = 0; i < sk_len; i++) {
    plain_private_key[65 - i] =
      test_case.private_key[test_case.private_key.size() - 1 - i];
  }

  uint8_t computed_shared[132] = { 0 };
  Hacl_P521_dh_responder(computed_shared, plain_public_key, plain_private_key);
  if (test_case.valid) {
    EXPECT_EQ(std::vector<uint8_t>(computed_shared, computed_shared + 66),
              test_case.shared);
  } else {
    EXPECT_NE(std::vector<uint8_t>(computed_shared, computed_shared + 66),
              test_case.shared);
  }
}
